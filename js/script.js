...
const loadData = async () => {
  ...
  categories.forEach((tabCategory) => {
    tabContainer.innerHTML += `
      <button onclick = "tabHandler('${tabCategory.category_id}',this)" class="bg-gray-300 rounded text-lg font-medium px-5 py-2  hover:bg-[#FF1F3D] hover:text-white">${tabCategory?.category}</button>
      `;
  });
};
...
const tabHandler = async (categoryId, own) => {
  ...
  const buttonContainer = document.getElementById('tab-container');
const allButton = buttonContainer.children;
const allButtonArray = [...allButton]
allButtonArray.forEach(e =>{
  e.classList.remove('bg-[#ff1f3d]')
  console.log(e);
})

// console.log(own);
own.classList.add('bg-[#ff1f3d]')
};
